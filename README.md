...
- Built-in OTLP/HTTP collector on `127.0.0.1:4318` — no external infrastructure required
...
- **Alerts** — configurable threshold-based alerts with per-agent context windows, agent-specific count thresholds, zero-cache detection, and badge count on the tab
- **Automation** — automated prompts triggered when session thresholds are crossed; configure per-agent automations for Loop Breaker, Turn Limit Wrap-up, and Context Dump; VS Code mode sends a notification or opens chat directly; standalone mode uses a file-based relay
- **Summaries** — human-readable session waterfall with LLM decisions, tool arguments and results, token usage per step, and background overhead breakdown
- **Traces** — raw OTLP span waterfall with full parent-child nesting, timing bars, and expandable attribute panels
- **Files** — files created or modified per session with inline before/after diffs
- **Agents** — side-by-side comparison of Copilot, Claude, and Codex with per-agent token totals, cache rates, time-to-first-token, and top tools; full session history table
- **Tokens** — token consumption by operation type and per session
