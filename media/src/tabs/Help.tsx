  ...
  return (
    <div class="help-section" id="help-sessions">
      ...
      <div class="help-overview-body">
        ...
        <p>Claude Code, Codex, and Copilot Chat (VS Code) each write one log file per work session on disk — but a single file can span multiple genuinely separate conversations if a long idle gap (30+ minutes) separates them, so AgentLens splits it into one session card per conversation rather than showing one entry with a misleading multi-hour (or multi-day) duration. A colored bar on the left edge of a row marks sessions that came from the same original file — same color means same conversation, split apart by time. Hover the bar for its position (e.g. "Part 2 of 5"), or click it to filter the table down to just that conversation's sessions — a banner appears above the filter bar with a "×" to clear it. Only sessions still visible under the active filters are colored; if a filter hides a sibling, the remaining row isn't colored — nothing implies a hidden sibling exists.</p>
