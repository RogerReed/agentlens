    ...
    <div style="position:relative;margin-bottom:8px">
      <svg viewBox={`0 0 ${W} ${H}`} style="width:100%;height:190px;display:block" onMouseLeave={() => setHovered(null)}>
        ...
        <text
          ...
          text-anchor="middle" fill="var(--vscode-charts-purple,#ba68c8)" font-size="9"
        >Cost</text>
        ...
        {[0, 1, 2, 3, 4].map(i => (
          <text key={i} x={W - pad.right + 4} y={pad.top + chartH * (1 - i / 4)} text-anchor="start" dominant-baseline="middle" fill="var(--vscode-charts-purple,#ba68c8)" font-size="9">
        ...
        {/* Cost line overlay */}
        {rows.length > 1 && (
          <polyline points={costPoints.join(' ')} fill="none" stroke="var(--vscode-charts-purple,#ba68c8)" stroke-width="1.5" stroke-dasharray="3 2" opacity="0.9" />
        )}
        {rows.map((r, i) => {
          if (r.costUsd === 0) return null
          const cy = pad.top + chartH * (1 - r.costUsd / maxCost)
          return <circle key={i} cx={cx(i)} cy={cy} r="1.5" fill="var(--vscode-charts-purple,#ba68c8)" />
        })}
      </svg>
      ...
      <div style="display:flex;gap:12px;font-size:10px;color:var(--muted);margin-top:2px;flex-wrap:wrap">
        {([
          ...
          ['var(--vscode-charts-purple,#ba68c8)', 'Cost',          true],
...
export function CostBarChart({ sessions, mode }: { sessions: SessionSummaryCard[]; mode: PricingMode }) {
  ...
  useEffect(() => {
    ...
    // Right Y axis — daily total (purple)
    ctx.fillStyle = 'rgba(186,104,200,0.85)'; ctx.textAlign = 'left'
    for (let i = 0; i <= 4; i++) {
